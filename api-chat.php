...
function matchLocalIntent($userMessage, $questionsData = []) {
    ...
    // GENERAL DIRECT PROJECT ASK (FLOWCHART: "To help you with any such information, first of all I have to know what is your project about")
    if ((preg_match('/\b(do my project|do a project|do this project|execute my project|execute a project|build my project|build a project|make a project|have a project|start a project|handle my project|take my project|construction help|project help|need help with|can you help|want to make a project|help me with|help with a project|help me build|can you do|can u do)\b/i', $msg) || (strpos($msg, 'project') !== false && (strpos($msg, 'do') !== false || strpos($msg, 'help') !== false || strpos($msg, 'build') !== false || strpos($msg, 'make') !== false))) && !preg_match('/\b(highway|bridge|flyover|peb|shed|turnkey|epc|school|hospital|township|residential|budget|cost|price|rate|discount|manpower|timeline|material)\b/i', $msg)) {
        return [
